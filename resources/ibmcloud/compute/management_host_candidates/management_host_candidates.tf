...
locals {
  instance = [{
    name                      = var.vsi_name
    primary_network_interface = ibm_is_instance.management_node.primary_network_interface[0].primary_ip.0.address
    }
  ]
  dns_record_ttl = 300
...
output "management_candidate_id" {
...
output "name" {
  value      = ibm_is_instance.management_node.name
  depends_on = [ibm_dns_resource_record.dns_record_record_a, ibm_dns_resource_record.dns_resource_record_ptr]
}