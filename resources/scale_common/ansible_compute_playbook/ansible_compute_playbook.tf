...
/*
    ...
    install IBM Spectrum Scale compute cluster.
*/
...
locals {
  ...
  compute_instances_root_key_path = format("%s/%s", var.tf_data_path, "id_rsa")
...
// create compute_tf_inventory.json file for gpfs setup on compute nodes
...
resource "null_resource" "prepare_ansible_inventory" {
  provisioner "local-exec" {
    ...
    command     = "python3 ${local.ansible_inv_script_path} --tf_inv_path ${local.tf_inv_path} --scale_cluster_def_path ${local.scale_cluster_def_path} --scale_tuning_profile_file ${local.scale_tuning_param_path} --ansible_ssh_private_key_file ${local.compute_instances_root_key_path}"
  }
...
// add 600 permission to compute ssh-key path
resource "null_resource" "check_key_permissions" {
  provisioner "local-exec" {
    interpreter = ["/bin/bash", "-c"]
    command     = "chmod 600 ${local.compute_instances_root_key_path}"
  }
  triggers = {
    builds = timestamp()
  }
...
// Invoke ansible paly book to setup gpfs compute cluster
